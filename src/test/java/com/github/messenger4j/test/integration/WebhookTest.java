...
class WebhookTest {
  ...
  private final Messenger messenger =
      Messenger.create("test", "60efff025951cddde78c8d03de52cc90", "CUSTOM_VERIFY_TOKEN");
  ...
  void shouldHandleAttachmentMessageEvent() throws Exception {
    ...
    assertThat(
        firstAttachment.asRichMediaAttachment().type(), equalTo(RichMediaAttachment.Type.IMAGE));
    ...
    JSONAssert.assertEquals(
        fallbackAttachmentJson, secondAttachment.asFallbackAttachment().json(), true);
  ...
  void shouldHandleTemplateEchoMessageEvent() throws Exception {
    // given
    final String payload =
        "{\"object\":\"page\",\"entry\":[{\"id\":\"171999997131834678\",\"time\":1480120722215,"
  ...
  void shouldHandlePostbackEvent() throws Exception {
    ...
    assertThat(
        postbackEvent.referral().get().refPayload(),
        equalTo(of("<REF DATA IF SPECIFIED IN THE AD>")));
  ...
  void shouldHandleMeLinkReferralEvent() throws Exception {
    ...
    assertThat(
        referralEvent.referral().refPayload(), equalTo(of("<REF DATA PASSED IN M.ME PARAM>")));
  }
  ...
  void shouldHandleAdReferralEvent() throws Exception {
    ...
    assertThat(referralEvent.referral().type(), equalTo("OPEN_THREAD"));
    assertThat(
        referralEvent.referral().refPayload(), equalTo(of("<REF DATA IF SPECIFIED IN THE AD>")));
  ...
  void shouldHandleAccountLinkingEventWithStatusLinked() throws Exception {
    ...
    assertThat(
        accountLinkingEvent.authorizationCode(), equalTo(of("PASS_THROUGH_AUTHORIZATION_CODE")));
  }
  ...
  void shouldHandleMessageDeliveredEventWithMids() throws Exception {
    ...
    assertThat(
        messageDeliveredEvent.messageIds().get().get(0),
        equalTo("mid.1458668856218:ed81099e15d3f4f233"));
  }
  ...
  void shouldThrowExceptionIfSignatureIsInvalid() {
    ...
    // when
    Assertions.assertThrows(
        MessengerVerificationException.class,
        () -> messenger.onReceiveEvents(payload, of(signature), mockEventHandler));
  ...
  void shouldThrowExceptionIfVerifyModeIsInvalid() {
    ...
    // when
    Assertions.assertThrows(
        MessengerVerificationException.class, () -> messenger.verifyWebhook(mode, verifyToken));
