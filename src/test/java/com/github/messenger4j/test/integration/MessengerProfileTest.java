...
class MessengerProfileTest {
  ...
  private final HttpResponse fakeResponse =
      new HttpResponse(200, "{\"result\": \"Successfully added new_thread's CTAs\"}");
  ...
  void beforeEach() throws Exception {
    when(mockHttpClient.execute(any(HttpMethod.class), anyString(), any()))
  ...
  void shouldDeleteStartButton() throws Exception {
    ...
    verify(mockHttpClient)
        .execute(eq(DELETE), endsWith(PAGE_ACCESS_TOKEN), payloadCaptor.capture());
    JSONAssert.assertEquals(expectedJsonBody, payloadCaptor.getValue(), true);
  }
  ...
  void shouldHandleUpdateSuccessResponse() throws Exception {
    ...
    final MessengerSettings messengerSettings =
        MessengerSettings.create(
            of(StartButton.create("test")), empty(), empty(), empty(), empty(), empty(), empty());
