...
class SendTest {
  ...
  private final HttpResponse fakeResponse =
      new HttpResponse(
          200,
          "{\n"
  ...
  private final Messenger messenger =
      Messenger.create(PAGE_ACCESS_TOKEN, "test", "test", of(mockHttpClient));

  @BeforeEach
  void beforeEach() throws Exception {
    when(mockHttpClient.execute(any(HttpMethod.class), anyString(), anyString()))
  ...
  void shouldSendTextMessageWithPhoneNumberRecipientWithName() throws Exception {
    ...
    final MessagePayload payload =
        MessagePayload.create(recipient, MessagingType.RESPONSE, TextMessage.create(text));

    messenger.send(payload);

    final ArgumentCaptor<String> payloadCaptor = ArgumentCaptor.forClass(String.class);
    final String expectedJsonBody =
        "{\"recipient\":{\"phone_number\":\"+1 (555) 857-6309\", "
  ...
  void shouldSendTextMessageWithEmailAndPhoneNumberQuickReplies() throws Exception {
    ...
    final List<QuickReply> quickReplies =
        Arrays.asList(userEmailQuickReply, userPhoneNumberQuickReply);
  ...
  void shouldSendGenericTemplateMessageWithLogInAndLogOutButtons() throws Exception {
    ...
    final Element element =
        Element.create(
            "Welcome to M-Bank",
            empty(),
            of(new URL("http://www.example.com/images/m-bank.png")),
            empty(),
            of(buttons));
  ...
  void shouldSendGenericTemplateMessageWithShareButton() throws Exception {
    ...
    final Element element =
        Element.create(
            "Breaking News: Record Thunderstorms",
            of("The local area is due for record thunderstorms over the weekend."),
            of(new URL("https://thechangreport.com/img/lightning.png")),
            empty(),
  ...
  void shouldSendGenericTemplateMessageWithShareButtonWithShareContents() throws Exception {
    ...
    final GenericTemplate shareGenericTemplate =
        GenericTemplate.create(
            singletonList(
                Element.create(
                    ...
                    of(
                        DefaultAction.create(
                            new URL("http://m.me/petershats?ref=invited_by_24601"))),
                    of(
                        singletonList(
                            UrlButton.create(
                                "Take Quiz",
  ...
  void shouldHandleSuccessResponse() throws Exception {
    final HttpResponse successfulResponse =
        new HttpResponse(
            200,
            "{\n"
                + "  \"recipient_id\": \"USER_ID\",\n"
  ...
  void shouldHandleErrorResponse() throws Exception {
    ...
    try {
      final MessagePayload payload =
          MessagePayload.create("test", MessagingType.RESPONSE, TextMessage.create("test"));
      messenger.send(payload);
